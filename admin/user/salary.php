...
add_page_info( 'title', extra_get_date($monthly->date, 'str: F Y').' Maaş' );
...
add_page_info( 'nav', array('name'=> extra_get_date($monthly->date, 'str: F Y').' Maaş') );
...
<div class="row">
	...
	<div class="col-lg-3 col-md-4">
		<div class="panel panel-warning panel-heading-0">
			<div class="panel-body">
				<small class="text-muted"><?php echo extra_get_date($monthly->date, 'F Y'); ?> ayına ait maaş tutarını değistir.</small>
				<div class="h-10"></div>
	...
	<div class="col-lg-9 col-md-8">
		...
		<div class="panel panel-default panel-table">
			<div class="panel-heading"><h3 class="panel-title"><?php echo extra_get_date($monthly->date, 'F Y'); ?> Maaş Detayları</h3></div>
					...
					<tbody>
						<?php while($item = $q_form_items->fetch_object() ): ?>
							<tr>
								...
								<td class="fs-11 text-muted"><?php echo extra_get_date($item->date, 'datetime'); ?></td>
								<td>
									<?php if($item->item_name == 'monthly_day'): ?>
										<?php echo extra_get_date($item->val_1, 'str: F Y'); ?>, günlük yevmiye
										<br />
										<small class="text-muted"><?php echo $item->val_2; ?> / <?php echo $item->val_3; ?></small>
									<?php elseif($item->item_name == 'overtime'): ?>
										Fazla mesai, <small class="text-muted">+<?php echo $item->quantity; ?> saat, <?php echo extra_get_date($item->val_date,'date'); ?></small>
										<br />
										<small class="text-muted"><?php echo $item->val_3; ?></small>
									<?php elseif($item->item_name == 'not_overtime'): ?>
										Eksik mesai, <small class="text-muted">-<?php echo $item->quantity; ?> saat, <?php echo extra_get_date($item->val_date,'date'); ?></small>
										<br />
										<small class="text-muted"><?php echo $item->val_3; ?></small>
