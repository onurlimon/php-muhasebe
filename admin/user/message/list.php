...
## mesaj kutusu secimi ve sayfa bilgilendirme
if(isset($_GET['box'])) {
	$box = 'inbox';
								...
								<a href="detail.php?id=<?php echo $list->id; ?>">
									...
									<?php if($q_select = db()->query("SELECT * FROM ".dbname('messages')." WHERE top_id='".$list->id."' ORDER BY date_update DESC LIMIT 1")) : ?>
										<?php if($q_select->num_rows) : $sub_message = $q_select->fetch_object(); ?>
											<span class="text-muted hidden-portrait"><b>-</b> <span class="underline"><?php echo _b(extra_get_strtolower(get_user_info($sub_message->sen_u_id, 'surname')),false); ?></span>: <?php echo mb_substr(strip_tags($sub_message->message),0,80,'utf-8'); ?><?php if(strlen($sub_message->message) > 80): ?>...<?php endif; ?></span>
										<?php endif; ?>
									<?php endif; ?>
								</a>
							</td>
