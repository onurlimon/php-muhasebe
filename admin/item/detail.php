...
## fotograf silme
if(isset($_GET['delete_item_image']) and user_access('admin')) {
	if($item->item_image) {
		if(delete_image($item->item_image)) {
			update_item($item->id, array('item_image'=>''));
		}
	}
}

## fotograf yukleme
if(isset($_FILES['item_image']['name']) and user_access('admin')) {
	...
	if($_FILES['item_image']['size'] > 0) {
		...
		if($upload_image = upload_item_image($_FILES['item_image'], $args)) {
			$_item['item_image'] = $upload_image;
			db()->query("UPDATE ".dbname('items')." SET item_image = '".$_FILES['item_image']['name']."' WHERE id='".$item->id."' ");
	...
	if(!is_alert('update_item')) {
		if(update_item($item->id, $_item)) {
...
<div class="tab-content"> 
	...
	<div class="tab-pane fade active in" role="tabpanel" id="home" aria-labelledby="home-tab"> 
		...
		<div class="row">
			<form name="form_add_accout" id="form_add_account" enctype="multipart/form-data" action="" method="POST" class="validate">
			
			<div class="col-md-2">
				<br />

				<div class="img-thumbnail">
					<img src="<?php if(!empty($item->item_image)){  echo '../../content/upload/product/'?><?php echo $item->item_image; } else {  template_url('img/hazirlaniyor.jpg');  } ?>" class="img-responsive" id="item_image_view" onclick="document.getElementById('item_image').click()" style="width:120px; height:180px;">
					<?php if(user_access('admin')): ?>
						<div class="h-10"></div>
					<!--	<a href="?id=<?php echo $item->id; ?>&delete_avatar" class="pull-right text-danger fs-12"><i class="fa fa-trash"></i> Bu fotoğrafı sil</a> -->
					<?php endif; ?>
				</div>
				...
				<?php if(user_access('admin')): ?>
					...
					<div class="form-group hidden">
						<label for="item_image">Yeni bir fotoğraf yükle.</label>
						<input  >
						<input type="FILE" name="item_image" id="item_image" onchange="render_form_file(this, function(img) { document.getElementById('item_image_view').src = img; });">
			...
			<div class="col-md-6">
				<?php print_alert('update_item'); ?>
					...
					<div class="text-right">
						...
						<button class="btn btn-primary btn-xs-block btn-insert"><i class="fa fa-floppy-o"></i> Kaydet</button>
					</div>
				...
				<div class="row">
					<div class="col-md-12">
						<?php 
							...
							$chart['options']['tooltips']['callbacks']['title'] = "=EXTRA= function(tooltipItems, data) { return ''; } =EXTRA=";
							$chart['options']['tooltips']['callbacks']['label'] = "=EXTRA= function(tooltipItems, data) {  return  data.labels[tooltipItems.index]+':      '+ tooltipItems.yLabel; } =EXTRA=";
									...
									<tbody>
										<?php foreach($whichAccount as $key=>$value): ?>
											...
											<tr onclick="location.href='<?php site_url('account', $account->id); ?>';" class="pointer">
												<td>
													<?php if($key): ?>
														<a href="<?php site_url('account', $account->id); ?>" title="<?php echo $account->name; ?>"><?php echo extra_get_substr($account->name, 0, 20); ?> <i class="fa fa-external-link"></i></a> 
													<?php else: ?>
														DİĞER
													<?php endif; ?>
												</td>
											...
											<tr onclick="location.href='<?php site_url('form', $list->form_id); ?>';" class="pointer">
												...
												<td>
													<?php if($list->account_id): ?>
														<a href="<?php site_url('account', $list->account_id); ?>" title="<?php echo get_form($list->form_id)->account_name; ?>"><?php echo extra_get_substr(get_form($list->form_id)->account_name, 0, 40); ?> <i class="fa fa-external-link"></i></a> 
													<?php else: ?>
														<?php echo extra_get_substr(get_form($list->form_id)->account_name, 0, 40); ?>
													<?php endif; ?>
												</td>
					...
					<?php while($list = $q_form_items->fetch_object()): ?>
						<?php if(extra_is_mobile()): ?>
							<tr id="<?php echo $list->id; ?>">
								<td class="hidden-xs-portrait fs-11 text-muted"><a href="<?php site_url('form', $list->form_id); ?>" target="_blank" style="text-decoration:none;"><?php echo extra_get_date($list->date, 'Y-m-d'); ?></a></td>
								<td><a href="<?php site_url('form', $list->form_id); ?>" target="_blank" style="text-decoration:none;">#<?php echo $list->form_id; ?></a></td>
								...
								<td class="text-center"><a href="<?php site_url('form', $list->form_id); ?>" target="_blank" style="text-decoration:none;"><?php echo $list->quantity; ?></a></td>
								<td class="text-right hidden-xs-portrait"><a href="<?php site_url('form', $list->form_id); ?>" target="_blank" style="text-decoration:none;"><?php echo get_set_money($list->price, true); ?></a></td>
								<td class="text-right"><a href="<?php site_url('form', $list->form_id); ?>" target="_blank" style="text-decoration:none;"><?php echo $list->in_out == '0' ? get_set_money($list->total, true) : ''; ?></a></td>
								<td class="text-right"><a href="<?php site_url('form', $list->form_id); ?>" target="_blank" style="text-decoration:none;"><?php echo $list->in_out == '1' ? get_set_money($list->total, true) : ''; ?></a></td>
							</tr>
						<?php else: ?>
							<tr id="<?php echo $list->id; ?>">
								<td class="text-muted fs-11"><a href="<?php site_url('form', $list->form_id); ?>" target="_blank" style="text-decoration:none;"><?php echo extra_get_date($list->date, 'Y-m-d H:i'); ?></a></td>
								<td><a href="<?php site_url('form', $list->form_id); ?>" target="_blank" style="text-decoration:none;">#<?php echo $list->form_id; ?></a></td>
								<td><a href="<?php site_url('form', $list->form_id); ?>" target="_blank" style="text-decoration:none;"><?php echo get_in_out_label($list->in_out); ?></a></td>
								<td>
									<?php if($list->account_id): ?>
										<?php echo get_account($list->account_id)->name; ?> <a href="<?php site_url('account', $list->account_id); ?>" target="_blank" class="fs-12"><i class="fa fa-external-link"></i></a>
									<?php endif; ?>
								...
								<td class="text-center"><a href="<?php site_url('form', $list->form_id); ?>" target="_blank" style="text-decoration:none;"><?php echo $list->quantity; ?></a></td>
								<td class="text-right"><a href="<?php site_url('form', $list->form_id); ?>" target="_blank" style="text-decoration:none;"><?php echo get_set_money($list->price, true); ?></a></td>
								<td class="text-right"><a href="<?php site_url('form', $list->form_id); ?>" target="_blank" style="text-decoration:none;"><?php echo $list->in_out == '0' ? get_set_money($list->total, true) : ''; ?></a></td>
								<td class="text-right"><a href="<?php site_url('form', $list->form_id); ?>" target="_blank" style="text-decoration:none;"><?php echo $list->in_out == '1' ? get_set_money($list->total, true) : ''; ?></a></td>
							</tr>
						<?php endif; ?>
