...
<div class="row">
							...
							<tbody>
								<?php while($list = $query->fetch_object()): ?>
									<tr onclick="location.href='<?php site_url('admin/item/detail.php?id='.$list->id); ?>';" class="pointer">
										<td><a href="<?php site_url('admin/item/detail.php?id='.$list->id); ?>" style="text-decoration:none;"><?php echo extra_get_substr($list->name, 0, 26); ?></a></td>
	...
	<div class="col-md-4">
		...
		if($query->num_rows) {
			...
			while($list = $query->fetch_object()) {
				if($i < 7) {
					$chart['data']['labels'][] = extra_get_substr($list->name,0,10);
...
<div class="row">
						...
						<table class="table table-hover table-condensed table-stripedd">
							...
							<tbody>
								<?php while($list = $query->fetch_object()): ?>
									...
									<tr class="">
										<td class="text-muted">
											<a href="<?php site_url('form', $list->form_id); ?>&flashItemID=<?php echo $list->item_id; ?>" target="_blank" style="text-decoration:none;">
												<?php 
													echo extra_get_date($list->date, 'd F'); 
												?>
											</a>
										</td>
										<td width="80"><a href="<?php site_url('form', $list->form_id); ?>&flashItemID=<?php echo $list->item_id; ?>" target="_blank" style="text-decoration:none;">#<?php echo $list->form_id; ?></a></td>
										<td><a href="<?php site_url('admin/item/detail.php?id='.$item->id); ?>" title="<?php echo $item->name; ?>" style="text-decoration:none;"><?php echo !extra_is_mobile() ? extra_get_substr($item->name, 0, 40) : extra_get_substr($item->name, 0, 16); ?></a></td>
										<?php if($list->in_out == '1'): ?>
											<td class="text-muted"><a href="<?php site_url('form', $list->form_id); ?>&flashItemID=<?php echo $list->item_id; ?>" target="_blank" style="text-decoration:none;">Çıkış</a></td>
										<?php else: ?> 
											<td class="text-muted"><a href="<?php site_url('form', $list->form_id); ?>&flashItemID=<?php echo $list->item_id; ?>" target="_blank" style="text-decoration:none;">Giriş</a></td>
										<?php endif; ?>
										<td class="text-center"><a href="<?php site_url('form', $list->form_id); ?>&flashItemID=<?php echo $list->item_id; ?>" target="_blank" style="text-decoration:none;"><?php echo $list->quantity; ?></a></td>
										<td class="text-right hidden-xs" width="100"><a href="<?php site_url('form', $list->form_id); ?>&flashItemID=<?php echo $list->item_id; ?>" target="_blank" style="text-decoration:none;"><?php echo get_set_money($list->price); ?> <i class="fa fa-try text-muted"></i></a></td>
										<td class="text-right hidden-xs" width="100"><a href="<?php site_url('form', $list->form_id); ?>&flashItemID=<?php echo $list->item_id; ?>" target="_blank" style="text-decoration:none;"><?php echo get_set_money($list->total); ?> <i class="fa fa-try text-muted"></i></a></td>
									</tr>
								<?php endwhile; ?>
							</tbody>
						</table>
	...
	<div class="col-md-4">
		...
		$chart['options']['maintainAspectRatio'] = false;
		$chart['options']['tooltips']['callbacks']['title'] = "=EXTRA= function(tooltipItems, data) { return data.labels[tooltipItems[0].index] + ''; } =EXTRA=";
		$chart['options']['tooltips']['callbacks']['label'] = "=EXTRA= function(tooltipItems, data) { return parseFloat(data.datasets[0].data[tooltipItems.index]) + ' ADET'; } =EXTRA=";
