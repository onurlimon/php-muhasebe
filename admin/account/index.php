	...
	<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
		...
		<div class="row space-none">
			...
			<div class="col-xs-12 col-md-6">
				...
				$chart['options']['tooltips']['callbacks']['label'] = "=EXTRA= function(tooltipItems, data) { return data.datasets[tooltipItems.datasetIndex].label +' : '+ tooltipItems.yLabel.formatMoney(2, '.', ',') + ' TL'; } =EXTRA=";
							...
							<tbody>
								<?php while($list = $query->fetch_object()): ?>
									<tr onclick="location.href='<?php site_url('account', $list->id); ?>';" class="pointer">
										<td><a href="<?php site_url('account', $list->id); ?>" title="<?php echo $list->name; ?>" style="text-decoration:none;"><?php echo extra_get_substr($list->name,0,30); ?></a></td>
										<td class="text-right" width="120"><a href="<?php site_url('account', $list->id); ?>" title="<?php echo $list->name; ?>" style="text-decoration:none;"><?php echo get_set_money($list->profit); ?> <small class="text-muted">TL</small></a></td>
									</tr>
								<?php endwhile; ?>
							</tbody>
						</table>
						...
						<table class="table table-hover table-condensed table-stripe">
							...
							<tbody>
								<?php while($list = $query->fetch_object()): ?>
									<tr onclick="location.href='<?php site_url('account', $list->id); ?>';" class="pointer">
										<td><a href="<?php site_url('account', $list->id); ?>" title="<?php echo $list->name; ?>" style="text-decoration:none;"><?php echo extra_get_substr($list->name,0,30); ?></a></td>
										<td class="text-right"><a href="<?php site_url('account', $list->id); ?>" title="<?php echo $list->name; ?>" style="text-decoration:none;"><?php echo get_set_money($list->balance); ?> <small class="text-muted">TL</small></a></td>
									</tr>
								<?php endwhile; ?>
							</tbody>
						</table>
							...
							<tbody>
								<?php while($list = $query->fetch_object()): ?>
									<tr onclick="location.href='<?php site_url('form', $list->id); ?>';" class="pointer">
										<td class="text-muted"><?php echo extra_get_date($list->date, 'd F'); ?></td>
										...
										<td><a href="<?php site_url('account', $list->account_id); ?>" title="<?php echo $list->account_name; ?>"><?php echo extra_get_substr($list->account_name,0,30); ?></a></td>
