	...
	<div class="tab-pane fade active in" role="tabpanel" id="home" aria-labelledby="home-tab"> 
			...
			<div class="col-md-8">
				...
				<form name="form_add_accout" id="form_add_account" action="" method="POST" class="validate">
					...
					<div class="text-right">
						...
						<button class="btn btn-primary btn-insert btn-xs-block"><i class="fa fa-floppy-o"></i> Kaydet</button>
					</div>
		...
		<div class="row">
			<div class="col-md-8">
				...
				$chart['options']['scales']['yAxes'][0]['ticks']['userCallback'] = "=EXTRA= function(value, index, values) { if(value >= 1) { return value + ' TL'; } } =EXTRA=";
			...
			<div class="col-md-4">
				...
				foreach($_data as $key=>$val) {
					if($i < 7) {
						$chart['data']['labels'][] = extra_get_substr(get_item($key)->name,0,10);
				...
				$chart['options']['tooltips']['callbacks']['label'] =  "=EXTRA= function(tooltipItems, data) { return parseFloat(data.datasets[0].data[tooltipItems.index]).formatMoney(2, '.', ',') + ' TL'; } =EXTRA=";
			...
			<table class="table table-hover table-bordered table-condensed table-striped dataTable">
				...
				<?php $balance = 0; foreach($forms->list as $form): ?>
					...
					<?php if(extra_is_mobile()): ?>
						<tr>
							<td>
								<a href="../form/detail.php?id=<?php echo $form->id; ?>" target="_blank" style="text-decoration:none;">#<?php echo $form->id; ?></a>
							</td>
							<td class="hidden-xs-portrait"><a href="../form/detail.php?id=<?php echo $form->id; ?>" target="_blank" style="text-decoration:none;"><small class="text-muted"><?php echo extra_get_date($form->date, 'Y-m-d'); ?></small></a></td>
							<td class="hidden-xs-portrait"><a href="../form/detail.php?id=<?php echo $form->id; ?>" target="_blank" style="text-decoration:none;"><small class="text-muted"><?php echo get_user_info($form->user_id, 'display_name'); ?></small></a></td>
							<td class="text-right"><a href="../form/detail.php?id=<?php echo $form->id; ?>" target="_blank" style="text-decoration:none;"><?php if($form->in_out == 0) { echo get_set_money($form->total, true); $balance = $balance - $form->total; } else { echo ''; } ?></a></td>
							<td class="text-right"><a href="../form/detail.php?id=<?php echo $form->id; ?>" target="_blank" style="text-decoration:none;"><?php if($form->in_out == 1) { echo get_set_money($form->total, true); $balance = $balance + $form->total; } else { echo ''; } ?></a></td>
							<td class="text-right"><a href="../form/detail.php?id=<?php echo $form->id; ?>" target="_blank" style="text-decoration:none;"><?php echo get_set_money($balance, true); ?></a></td>
						</tr>
					<?php else: ?>
						<tr>
							<td>
							...
							<td><a href="../form/detail.php?id=<?php echo $form->id; ?>" target="_blank" style="text-decoration:none;"><small class="text-muted hidden-xs"><?php echo substr($form->date,0,16); ?></small></a></td>
							<td class="hidden-xs"><a href="../form/detail.php?id=<?php echo $form->id; ?>" target="_blank" style="text-decoration:none;"><?php echo get_in_out_label($form->in_out); ?></a></td>
							<?php if($form->type == 'form'): ?>
								<td class="hidden-xs"><a href="../form/detail.php?id=<?php echo $form->id; ?>" target="_blank" style="text-decoration:none;"><?php echo get_form_status_span($form_status); ?></a></td>
							<?php else: ?>
								<td class="hidden-xs"></td>
							<?php endif; ?>
							<td class="hidden-xs"><a href="../form/detail.php?id=<?php echo $form->id; ?>" target="_blank" style="text-decoration:none;"><?php echo get_user_info($form->user_id, 'display_name'); ?></a></td>
							<td class="text-muted hidden-xs">
								<a href="../form/detail.php?id=<?php echo $form->id; ?>" target="_blank" style="text-decoration:none;">
								<?php
								if($form->type=='form') {
