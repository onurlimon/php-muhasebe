...
function set_payment($id, $args=array()) {
	...
	if(!is_alert(__FUNCTION__)) {
		...
		$_form_meta['account_country'] 	= @extra_get_strtoupper(@$insert['account_country']);
		$_form_meta['account_district'] = @extra_get_strtoupper($insert['account_district']);
		$_form_meta['account_address'] 	= @extra_get_strtoupper($insert['account_address']);
		$_form_meta['note']	 			= @extra_get_strtoupper($insert['note']);
...
function update_case($where, $args) {
	...
	if(!is_alert(__FUNCTION__)) {
		...
		if(isset($_update['update']['val_5']) and isset($_update['update']['val_enum'])) {
			db()->query("UPDATE ".dbname('extra')." SET val_5='' WHERE taxonomy='extra_case' AND val_enum='".$_update['update']['val_enum']."' ");
		}
...
function _set_case_args($args, $return_args=false) {
	...
	if(!isset($args['taxonomy'])) { $_arr['taxonomy'] = 'extra_case'; }
	...
	if(isset($args['is_bank'])) { $_arr['val_enum'] = $args['is_bank']; }
	if(substr($_arr['taxonomy'], 0,6) != 'extra_') { $_arr['taxonomy'] = 'extra_'.$_arr['taxonomy'];}
