...
function pagination($count) {
	...
	$paged = ceil($count / extra()->pg->list_limit);
	if(isset($_GET['page'])) {
		$page = $_GET['page'];
	} else {
		$page = 1;
...
/**
 * get_theme_logs()
 * get_logs() fonksiyonundaki verileri "extra1" temasına gore suzer ve gosterir
 */
				...
				<tr>
					<?php if(extra_is_mobile()): ?>
						...
						<td>
							...
							<small class="text-muted"><?php echo $active->name."&nbsp;".$active->surname; ?></small>
						</td>
						<td>
					...
					<?php else: ?>
						...
						<td><span class="fs-12"><?php echo $active->name."&nbsp;".$active->surname; ?></span></td>
						<td>
							<span class="hidden"><?php echo $log->log_key; ?></span>
							<?php echo $log->log_text; ?>
							<?php if(isset($log->meta)): ?>
