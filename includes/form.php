...
function set_form($id, $args=array()) {
	...
	if(!have_log()) { // eger daha onceden eklenmemis ise log kaydi bulunmayacaktir.
		...
		if(!is_alert(__FUNCTION__)) {
			...
			$_form['account_name'] 		= @extra_get_strtoupper($set['account_name']);
			$_form['account_gsm'] 		= @$set['account_gsm'];
			$_form['account_phone'] 	= @$set['account_phone'];
			$_form['account_email'] 	= @extra_get_strtolower($set['account_email']);
			$_form['account_city'] 		= @extra_get_strtoupper($set['account_city']);
			$_form['account_tax_home']	= @extra_get_strtoupper($set['account_tax_home']);
			$_form['account_tax_no'] 	= @$set['account_tax_no'];



			$_form_meta['account_country'] 	= @extra_get_strtoupper(@$set['account_country']);
			$_form_meta['account_district'] = @extra_get_strtoupper($set['account_district']);
			$_form_meta['account_address'] 	= @extra_get_strtoupper($set['account_address']);
			$_form_meta['note']	 	= @extra_get_strtoupper($set['note']);
			...
			if(!empty($set['item_code']) or !empty($set['item_name'])) {
				...
				$form_item['item_name']		= extra_get_strtoupper($set['item_name']);
 ...
 */
function get_form($id_or_query, $_extra=true ) {
	if(is_array($id_or_query)) { $where = $id_or_query; } else { $where['id'] = $id_or_query; }

	if( isset($where['id']) and $_extra ) {
		if( isset(extra()->forms[$where['id']]) ) {
			return extra()->forms[$where['id']];
		}
	}
	...
	if(!have_log($args['uniquetime'])) { // eger daha onceden eklenmemis ise log kaydi bulunmayacaktir.
		...
		if(!empty($args['item_code'])) {
			if(!$item = get_item(array('code'=>$args['item_code']))) {
				...
				if($args["in_out"]){
					$quantities = db()->query("SELECT * FROM ".dbname('items')." WHERE id='".$item->id."'");
					$a = $quantities->fetch_array();
					if($a['quantity'] < $args['quantity'] ){  $args['quantity'] = $a['quantity'] ; }
				}
		...
		if($old_status = get_form_status($_where)) {
			...
			if($extra_id = update_extra($_where, $_args)) {
				if($extra = get_form_status($extra_id)) {
					$fs_cat_name = extra()->form_status[$extra->taxonomy]['name'];
...
function get_form_status_all($args=array()) {
	...
	if(!isset($where['taxonomy'])) { $where['taxonomy'] = 'extra_fs_form'; }
...
function delete_form_status($delete_status_id, $new_status_id, $args=array()) {
	...
	$fs_cat_name = extra()->form_status[$delete_status->taxonomy]['name'];
...
function _set_form_status_default($_taxonomy) {
	...
	if(substr($_taxonomy, 0,9) != 'extra_fs_') { $_taxonomy = 'extra_fs_'.$_taxonomy; }
	...
	if(extra()->form_status[$_taxonomy]['in_out']) {
...
function _set_form_status_args($args, $return_args=false) {
	...
	if(substr($_arr['taxonomy'], 0,9) != 'extra_fs_') { $_arr['taxonomy'] = 'extra_fs_'.$_arr['taxonomy']; }


	if(isset($_arr['name'])) { $_arr['name'] = extra_get_ucwords($_arr['name']); }
...
function is_form_status($taxonomy) {
	if(isset(extra()->form_status[$taxonomy])) {
		return true;
	} else {
		return false;
	}
