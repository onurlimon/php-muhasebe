...
function upload_image($file, $args=array()) {
	...
	if(!is_alert(__FUNCTION__) and !have_log(@$args['uniquetime'])) {
		...
		$file_to_upload = $path.'/extra-uid-'.get_active_user('id').'-dt-'.date('YmdHis').'.'.$extension;
...
function upload_item_image($file, $args=array()) {

	$args = _args_helper($args);

	$path = ROOT_PATH.'/content/upload/product/';
