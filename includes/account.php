	...
	if(!have_log()) { // eger daha onceden eklenmemis ise log kaydi bulunmayacaktir.
		...
		if(!is_alert(__FUNCTION__, 'danger')) { // eger herhangi bir hata yok ise
			...
			if($q_is_code->num_rows) { // eger veritabanında ayni barkod var ise
				...
				if(isset($insert['name'])) 		{ $insert['name'] = extra_get_strtoupper($insert['name']); }
				if(isset($insert['address'])) 	{ $insert['address'] = extra_get_strtoupper($insert['address']); }
				if(isset($insert['city'])) 		{ $insert['city'] = extra_get_strtoupper($insert['city']); }
				if(isset($insert['district'])) 	{ $insert['district'] = extra_get_strtoupper($insert['district']); }
				if(isset($insert['country'])) 	{ $insert['country'] = extra_get_strtoupper($insert['country']); }
				if(isset($insert['tax_home'])) 	{ $insert['tax_home'] = extra_get_strtoupper($insert['tax_home']); }
				if(isset($insert['email'])) 	{ $insert['email'] = extra_get_strtolower($insert['email']); }
 ...
 */
function get_account($args, $_extra=true) {
	...
	if( isset($where['id']) and $_extra ) {
		if( isset(extra()->accounts[$where['id']]) ) {
			return extra()->accounts[$where['id']];
		}
	}
	...
	if($query = db()->query("SELECT * FROM ".dbname('accounts')." ".sql_where_string($where)." ")) {
		if($query->num_rows) {
			...
			extra()->accounts[$return->id] = $return;
			return $return;
		} else {
		...
		if($old_account = get_account($where)) {
			...
			if(!is_alert(__FUNCTION__, 'danger')) { // eger herhangi bir hata yok ise
				...
				if($q_is_code->num_rows) { // eger veritabanında ayni barkod var ise
					...
					if(isset($update['name'])) 		{ $update['name'] = extra_get_strtoupper($update['name']); }
					if(isset($update['address'])) 	{ $update['address'] = extra_get_strtoupper($update['address']); }
					if(isset($update['city'])) 		{ $update['city'] = extra_get_strtoupper($update['city']); }
					if(isset($update['district'])) 	{ $update['district'] = extra_get_strtoupper($update['district']); }
					if(isset($update['country'])) 	{ $update['country'] = extra_get_strtoupper($update['country']); }
					if(isset($update['tax_home'])) 	{ $update['tax_home'] = extra_get_strtoupper($update['tax_home']); }
					if(isset($update['email'])) 	{ $update['email'] = extra_get_strtolower($update['email']); }
