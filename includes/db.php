 ...
 * gunvelik derken: mysqli_real_escape_string .
 */
...
function sql_insert_string($arr=array(), $opt=array())
{
	...
	if(count($arr['insert']) > 0) {
		$return['table_name'] = 'e_x_t_r_a_this_value_replace';
		$return['table_value'] = 'e_x_t_r_a_this_value_replace';
		...
		$return['table_name'] = str_replace('e_x_t_r_a_this_value_replace, ', '', $return['table_name']);
		$return['table_value'] = str_replace('e_x_t_r_a_this_value_replace, ', '', $return['table_value']);
...
function sql_update_string($arr=array(), $opt=array())
{
	...
	$return = 'e_x_t_r_a_this_value_replace'; // bu deger silinecek
	foreach($arr as $name=>$value)
	{
		$return = $return.', '.$name."='".$value."'";
	}

	return str_replace('e_x_t_r_a_this_value_replace, ', '', $return);
} //.sql_update_string()
    	...
    	foreach($arr as $key=>$val) {
      		if(is_numeric($key) and is_array($val)) {
        		foreach($val as $key1=>$val1) {
        			if(extra_get_strtoupper($key1) == 'ORDER_BY') { $order_by = $val1; }
        			elseif(extra_get_strtoupper($key1) == 'LIMIT') { $limit = $val1; }
        			else { $r .= _where_string($key1, $val1, $arr); }
        		}
      		} else {
      			if(extra_get_strtoupper($key) == 'ORDER_BY') { $order_by = $val; }
      			elseif(extra_get_strtoupper($key) == 'LIMIT') { $limit = $val; }
...
function _where_string($name, $val='', $arr=array()) {
  ...
  # OR
  if(extra_get_strtoupper($name) == 'OR') { // eger OR degeri var ise
    $r = ' OR ';
  }
  # IN
  elseif(extra_get_strtoupper($name) == 'IN') {
  ...
  # NOT IN
  elseif(extra_get_strtoupper($name) == 'NOT_IN') {
  ...
  # LIKE
  elseif(extra_get_strtoupper($name) == 'LIKE') {
  ...
  # NOT LIKE
  elseif(extra_get_strtoupper($name) == 'NOT_LIKE') {
  ...
  # ORDER BY
  elseif(extra_get_strtoupper($name) == 'ORDER_BY') {
    $r .= ' ORDER BY ';
    foreach($val as $name=>$val) { // sadece bir tane deger ise
      $r .= ''.$name." ".$val.", ";
    }
  ...
  # LIMIT
  elseif(extra_get_strtoupper($name) == 'LIMIT') {
    $r .= ' LIMIT '.$val;
    // fazlaliklari silelim
    if(substr($r, -2) == ', ') { $r = substr($r, 0, -2); }
  }
...
function get_ARR_helper_limit_AND_orderby($arr) {
	...
	if(!isset($arr['limit'])) 	{ $arr['limit'] = extra()->pg->list_limit; }
