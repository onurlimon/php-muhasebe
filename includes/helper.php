...
/**
 * extra_get_addslashes()
 * dizi halinde gelen tum degerlere addslashes ekler
 */
function extra_get_addslashes($arr) {
    $return = array();
    if( !is_array($arr) ) {
        $return = addslashes($arr);
         ...
         foreach($arr as $k=>$v) {
            if(is_array($v)) {
                $return[$k] = extra_get_addslashes($v);
            } else {
 ...
 *
 */
function extra_get_substr($val, $start, $end, $args='...') {
    if(!is_array($args)) {
        $dot3 = $args;
        $args = array();
        if($dot3) {
...
/**
 * extra_get_abbreviation()
 *
 */
function extra_get_abbreviation($str, $max_length=2) {
...
function extra_active_site_url() {
...
function extra_get_date_lang($val) {
 ...
 */
function extra_get_date($date, $args=array()) {
    $return = '';
    ...
    return extra_get_date_lang($return);
}
 ...
 */
function extra_get_ucwords($string, $e ='utf-8') {
...
/**
 * extra_get_strtoupper()
 * bir dizgeyi komple buyuk harf yapar
 */
function extra_get_strtoupper($string, $e ='utf-8') {
...
/**
 * extra_get_strtolower()
 * bir dizgeyi komple kucuk harf yapar
 */
function extra_get_strtolower($string, $e ='utf-8') {
...
/**
 * extra_get_money_convert_string()
 * bir rakamı yani parasal degeri string/text formatina cevirir
 */
function extra_get_money_convert_string($money='0.00') {
    $l9 = '';
    $l8 = '';
    $l7 = '';
    $l6 = '';
