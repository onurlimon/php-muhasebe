...
function add_log($arr=array()) {
	...
	$q_insert = db()->query("INSERT INTO ".dbname('logs')."  (date, uniquetime, table_id, user_id, log_url, log_key, log_text) VALUES 
		(
		'".extra_get_addslashes($arr['date'])."', 
		'".extra_get_addslashes($arr['uniquetime'])."', 
		'".extra_get_addslashes($arr['table_id'])."', 
		'".extra_get_addslashes($arr['user_id'])."', 
		'".extra_get_addslashes($arr['log_url'])."',
		'".extra_get_addslashes($arr['log_key'])."', 
		'".extra_get_addslashes($arr['log_text'])."'
		) ");
...
function add_console_log( $args, $function_name=false ) {
	...
	if(!$function_name) { $function_name = 'Extra'; }
    if ( is_array( $args ) )
