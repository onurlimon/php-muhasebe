...
function add_user($args) {
	...
	$insert = $args['insert'];
	$insert['gsm'] = str_replace("(","",$insert['gsm'] );
	$insert['gsm'] = str_replace(")","",$insert['gsm'] );
	$insert['gsm'] = str_replace("-","",$insert['gsm'] );
	$insert['gsm'] = str_replace(" ","",$insert['gsm'] );
	...
	if(!have_log()) {
		...
		@form_validation($insert['email'], 'email', 'E-posta', 'required|email', __FUNCTION__);
		...
		if(!is_alert(__FUNCTION__)) { // eger herhangi bir hata yok ise
			...
			// string transactions
			$insert['username']		= extra_get_strtolower($insert['username']);
			$insert['name']			= extra_get_strtoupper($insert['name']);
			$insert['surname']		= extra_get_strtoupper($insert['surname']);
...
function update_user($user_id, $args) {
	...
	if(!have_log()) { // bu islem daha onceden gercekles mi?
		...
		if(!is_alert(__FUNCTION__)) { // eger herhangi bir hata yok ise
			...
			// string transactions
			if(isset($update['username'])) 	{ $update['username']		= extra_get_strtolower($update['username']); 	}
			if(isset($update['name'])) 		{ $update['name']			= extra_get_strtoupper($update['name']); 	}
			if(isset($update['surname'])) 	{ $update['surname']		= extra_get_strtoupper($update['surname']); 	}
 ...
 */
function get_user($args, $_extra=true) {
	...
	if ( isset(extra()->user[$where['id']]) AND $_extra ) {
		return extra()->user[$where['id']];
	} else {
 ...
 * sadece kullanıcının gerçek adının almak icin get_active_user('name') veya get_active_user()->name seklinde kullanılabilir.
 */
...
function set_staff_salary($user_id) {
	...
	if(!$account = get_account($user->account_id, false)) {
		...
		$_account['insert']['code'] = 'EXTRAUA-'.$user->id;
